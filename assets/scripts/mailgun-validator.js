...
$.fn.mailgun_validator = function(options) {
    return this.each(function() {
        ...
        $(this).on('input',function(e){
            if (hasInput) {
                // change alter icon to greyed out & start verify countdown
                $('#emailValidity').html("<aside><i class='fa fa-dot-circle-o grey'></i></aside>");
                window.setTimeout(function() {
                    // #email because issues with settimeout and this
                    run_validator($("#email").val(), options); }, 2000);
            }
        });
    });
};
